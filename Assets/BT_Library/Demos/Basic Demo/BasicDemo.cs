...
public class BasicDemo : MonoBehaviour {
	...
	void Awake () {
		...
		/*
		 * We need to identfy the device either by its MAC Adress or Name (NOT BOTH! it will use only one of them to identefy your device).
		 *
		 *---------- MacAdress property
        ...
        //device.MacAddress = "CC:50:E3:80:A8:B8";
    ...
    void Update() {
		if (device.IsReading) {
            ...
            //int[] content = msg.Select(x => (int)x).ToArray();
            ...
            if (msg != null ) {
				 ...
				 */
                //string content2 = System.Text.ASCIIEncoding.ASCII.GetString(msg);
